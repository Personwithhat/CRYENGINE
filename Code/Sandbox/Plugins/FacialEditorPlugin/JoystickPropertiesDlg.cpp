	...
	: CDialog(IDD_FACEED_JOYSTICK_PROPERTIES, pParent)
{
	...
	m_scales[0] = m_scales[1] = 1;
	m_flippeds[0] = m_flippeds[1] = false;
	m_offsets[0] = m_offsets[1] = 0;
...
BOOL CJoystickPropertiesDlg::OnInitDialog()
{
	...
	m_scaleEdits[0].SetRange(-BADMP(1e+10), BADMP(1e+10));
	m_offsetEdits[0].Create(this, IDC_VIDEO_OFFSET_X);
	m_offsetEdits[0].SetRange(-BADMP(1e+10), BADMP(1e+10));

	m_scaleEdits[1].Create(this, IDC_VIDEO_SCALE_Y);
	m_scaleEdits[1].SetRange(-BADMP(1e+10), BADMP(1e+10));
	m_offsetEdits[1].Create(this, IDC_VIDEO_OFFSET_Y);
	m_offsetEdits[1].SetRange(-BADMP(1e+10), BADMP(1e+10));
...
void CJoystickPropertiesDlg::SetVideoScale(int axis, const mpfloat& offset)
{
	m_scales[axis] = offset;
}

const mpfloat& CJoystickPropertiesDlg::GetVideoScale(int axis) const
{
	return m_scales[axis];
}

void CJoystickPropertiesDlg::SetVideoOffset(int axis, const mpfloat& offset)
{
	m_offsets[axis] = offset;
}

const mpfloat& CJoystickPropertiesDlg::GetVideoOffset(int axis) const
{
