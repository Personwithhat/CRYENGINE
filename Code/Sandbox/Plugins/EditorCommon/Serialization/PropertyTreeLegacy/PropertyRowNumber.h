...
public: // Incrementation
	...
	void endIncrement(PropertyTreeLegacy* tree) override
	{
	...
	void increment(PropertyTreeLegacy* tree, int mouseDiff, Modifier modifier) override
	{
		mpfloat add = 0;
...
public: // More incrementation
	void add(PropertyTreeLegacy* tree) override { addValue(tree, singleStep_);  }
	void sub(PropertyTreeLegacy* tree) override { subValue(tree, singleStep_); }

	// PERSONAL NOTE: Has to be separate, can't pass in negative value since property might be Unsigned e.g. u64!
	void subValue(PropertyTreeLegacy* tree, const Type& value)
	{
		tree->model()->rowAboutToBeChanged(this);
		value_ = CLAMP(value_ - value, hardLimit_.min, hardLimit_.max);
		tree->model()->rowChanged(this, true);
	}
	void addValue(PropertyTreeLegacy* tree, const Type& value)
	{
		tree->model()->rowAboutToBeChanged(this);
		value_ = CLAMP(value_ + value, hardLimit_.min, hardLimit_.max);
		tree->model()->rowChanged(this, true);
