...
property_tree::InplaceWidget* PropertyRowNumberField::createWidget(PropertyTreeLegacy* tree)
{
...
bool PropertyRowNumberField::getHoverInfo(PropertyHoverInfo* hit, const Point& cursorPos, const PropertyTreeLegacy* tree) const
{
...
bool PropertyRowNumberField::onMouseDown(PropertyTreeLegacy* tree, Point point, bool& changed)
{
...
void PropertyRowNumberField::onMouseUp(PropertyTreeLegacy* tree, Point point) 
{
	tree->ui()->unsetCursor();
	pressed_ = false;
	dragStarted_ = false;
...
int PropertyRowNumberField::widgetSizeMin(const PropertyTreeLegacy* tree) const
{ 
	if (userWidgetSize() >= 0)
