...
void PlaybackPanel::OnTimelineCopy(const CTimeValue& time, STimelineTrack* pTrack)
{
...
void PlaybackPanel::OnTimelinePaste(const CTimeValue& time, STimelineTrack* pTrack)
{
	...
	if (!strcmp("Character Tool Animation Events", clipboard.GetTitle()) && clipboard.Get().isValid())
	{
		if (Serialization::LoadXmlNode(serializeProxy, clipboard.Get()) && !pastedEvents.empty())
		{
			...
			nTime offset = TimelineTimeToAnimEventTime(time) - pastedEvents[0].startTime;
...
void PlaybackPanel::OnPlaybackTimeChanged()
{
	...
	CTimeValue duration = m_system->document->MaxPlaybackDurationOfAllEnabledLayers();
