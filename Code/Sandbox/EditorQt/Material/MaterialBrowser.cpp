...
BOOL CMaterialToolBar::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{
		m_fIdleFilterTextTime = gEnv->pTimer->GetFrameStartTime();
	}
...
const CString& CMaterialToolBar::GetFilterText(bool* pOutIsNew)
{
	...
	if (m_fIdleFilterTextTime > 0 && gEnv->pTimer->GetFrameStartTime() - m_fIdleFilterTextTime > "0.3")
	{
		if (m_pEdit)
		{
	...
	case eNotify_OnIdleUpdate:
		{
			...
			if (m_fIdleSaveMaterialTime > "0.0001" && gEnv->pTimer->GetFrameStartTime() - m_fIdleSaveMaterialTime > 1)
			{
...
{
	m_fIdleSaveMaterialTime = gEnv->pTimer->GetFrameStartTime();
}
