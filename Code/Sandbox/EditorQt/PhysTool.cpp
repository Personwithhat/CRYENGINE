...
bool CPhysPullTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, int flags)
{
	...
	if (event == eMouseLDown)
	{
		if (flags & MK_SHIFT)
		{
			m_timeHit = gEnv->pTimer->GetFrameStartTime();
			return true;
		}
		if (nHits)
		{
			...
			m_timeMove = gEnv->pTimer->GetFrameStartTime();
...
void CPhysPullTool::Display(DisplayContext& dc)
{
	...
	if (m_timeHit != 0 && t > m_timeHit + "0.3")
	{
		...
		dc.DrawBall(SMiniCamera(dc.camera).Screen2World(m_lastMousePos, 1), (float)(min(CTimeValue(2), t - m_timeHit - "0.3") * "0.01").GetSeconds());
	}
...
void CPhysPullTool::UpdateAttachPos(const SMiniCamera& cam, const CPoint& point)
{
	if (m_pEntAttach && m_pEntPull)
	{
		...
		CTimeValue t = gEnv->pTimer->GetFrameStartTime();
		if (t > m_timeMove + gEnv->pTimer->GetFrameTime() * "0.5")
		{
			pe_action_set_velocity asv;
			asv.v = (pp.pos - m_lastAttachPos) / (t - m_timeMove).BADGetSeconds();
