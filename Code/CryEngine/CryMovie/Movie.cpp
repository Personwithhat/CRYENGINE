...
CTimeValue CMovieSystem::m_mov_cameraPrecacheTime = CTimeValue(1);
#if !defined(_RELEASE)
...
void CMovieSystem::ShowPlayedSequencesDebug()
{
	...
	for (Sequences::iterator it = m_sequences.begin(); it != m_sequences.end(); ++it)
	{
		...
		x += 100.0f;
		IRenderAuxText::Draw2dLabel(x, y, 1.2f, bIsRunning ? green : white, false, "%u", uint(playingTime.GetSeconds() * gEnv->pTimer->GetTicksPerSecond()));
