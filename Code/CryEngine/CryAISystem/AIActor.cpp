	...
	if (gAIEnv.CVars.collisionAvoidance.CollisionAvoidanceEnableRadiusIncrement)
	{
		if (m_pActor->m_State.fDesiredSpeed > 0.5f)
		{
			m_radiusIncrement = min(
				m_radiusIncrement + (m_pActor->m_movementAbility.collisionAvoidanceRadiusIncrement * gAIEnv.CVars.collisionAvoidance.CollisionAvoidanceRadiusIncrementIncreaseRate * updateTime.BADGetSeconds()),
		...
		else
		{
			m_radiusIncrement = max(
				m_radiusIncrement - (m_pActor->m_movementAbility.collisionAvoidanceRadiusIncrement * gAIEnv.CVars.collisionAvoidance.CollisionAvoidanceRadiusIncrementDecreaseRate * updateTime.BADGetSeconds()),
				0.0f
			);
		}
	}
