...
INavigationSystem::EWorkingState NavigationSystem::Update(const CTimeValue& frameStartTime, const CTimeValue& frameTime, bool blocking)
{
...
#if NAV_MESH_REGENERATION_ENABLED
void NavigationSystem::UpdateMeshes(const CTimeValue& frameStartTime, const CTimeValue& frameTime, const bool blocking, const bool multiThreaded, const bool bBackground)
{
...
void NavigationSystem::UpdateMeshesFromEditor(const bool blocking, const bool multiThreaded, const bool bBackground)
{
	...
	const CTimeValue frameDuration = "0.0333";
	UpdateMeshes(GetGTimer()->GetFrameStartTime(), frameDuration, blocking, multiThreaded, bBackground);
}
...
void NavigationSystem::RemoveAllTrianglesByFlags(const MNM::AreaAnnotation::value_type flags)
{
	...
	CryLog("Time used by removing NavMesh triangles: %li ms", (int64)(GetGTimer()->GetAsyncTime() - startTime).GetMilliSeconds());
}
...
void NavigationSystemDebugDraw::DebugDrawPathFinder(NavigationSystem& navigationSystem, const DebugDrawSettings& settings)
{
	...
	CTimeValue timeTotal(0);
	CTimeValue stringPullingTotalTime(0);
	...
	if (closestTriangleStart.id.IsValid() && closestTriangleEnd.id.IsValid())
	{
		...
		workingSet.aStarNodesList.SetFrameTimeQuota(0);
		...
		CTimeValue timeEnd = GetGTimer()->GetAsyncTime();
		...
		if (bPathFound)
		{
			...
			if (CMNMPathfinder::ConstructPathFromFoundWay(result, navMesh, offMeshNavigationManager, fixedPointStartLoc.GetVec3(), fixedPointEndLoc.GetVec3(), *&outputPath))
			{
				...
				CTimeValue stringPullingStartTime = GetGTimer()->GetAsyncTime();
				if (bBeautifyPath)
				{
					outputPath.PullPathOnNavigationMesh(navMesh, gAIEnv.CVars.pathfinder.PathStringPullingIterations, nullptr);
				}
				stringPullingTotalTime = GetGTimer()->GetAsyncTime() - stringPullingStartTime;
	...
	dc->Draw2dLabel(10.0f, 172.0f, 1.3f, Col_White, false,
		"Start: %08x  -  End: %08x - Total Pathfinding time: %.4fms -- Type of prediction for the point inside each triangle: %s", closestTriangleStart.id, closestTriangleEnd.id, (float)timeTotal.GetMilliSeconds(), predictionName.c_str());
	dc->Draw2dLabel(10.0f, 184.0f, 1.3f, Col_White, false,
		"String pulling operation - Iteration %d  -  Total time: %.4fms -- Total Length: %f", gAIEnv.CVars.pathfinder.PathStringPullingIterations, (float)stringPullingTotalTime.GetMilliSeconds(), totalPathLength);
}
...
void NavigationSystemBackgroundUpdate::Thread::ThreadEntry()
{
	while (!m_requestedStop)
	{
		...
		{
			const CTimeValue startedUpdate = GetGTimer()->GetAsyncTime();
