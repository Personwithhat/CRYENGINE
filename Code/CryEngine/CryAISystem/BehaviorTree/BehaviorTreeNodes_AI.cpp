...
protected:
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		{
			const CTimeValue timeSpentWithCorrectDirection = runtimeData.timeSpentWithCorrectDirection + gEnv->pTimer->GetFrameTime();
...
protected:
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		const CTimeValue timeSpentAligning = runtimeData.timeSpentAligning + gEnv->pTimer->GetFrameTime();
...
protected:
	virtual void OnInitialize(const UpdateContext& context) override
	{
		...
		runtimeData.timeWhenShootingShouldEnd = gEnv->pTimer->GetFrameStartTime() + m_duration;
		runtimeData.timeObstructed.SetSeconds(0);
	}
