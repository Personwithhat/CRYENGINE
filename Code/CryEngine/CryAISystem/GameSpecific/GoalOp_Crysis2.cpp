...
EGoalOpResult COPCrysis2FlightFireWeapons::Execute(CPipeUser* pPipeUser)
{
	...
	case eFP_WAIT_INIT:
		...
		{
			m_WaitTime = gEnv->pTimer->GetFrameStartTime() + cry_random(m_minTime, m_maxTime);
			m_State = eFP_WAIT;
		}
		else
		{
	...
	case eFP_PREPARE:
		{
			if (m_State == eFP_WAIT && gEnv->pTimer->GetFrameStartTime() > m_WaitTime)
			{
				pPipeUser->m_State.fire = eAIFS_Off;
				pPipeUser->m_State.fireSecondary = eAIFS_Off;
				m_State = m_NextState;
