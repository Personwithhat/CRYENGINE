...
{
	CTimeValue fTargetStayAliveTime = gAIEnv.CVars.legacyPuppetRod.RODAliveTime;
	...
	else
	{
		...
		const CTimeValue fMoveInc = gAIEnv.CVars.legacyPuppetRod.RODMoveInc;
		{
		...
		const CTimeValue fStanceInc = gAIEnv.CVars.legacyPuppetRod.RODStanceInc;
		{
			CTimeValue fIncrease;
		...
		const CTimeValue fDirectionInc = gAIEnv.CVars.legacyPuppetRod.RODDirInc;
		{
			CTimeValue fIncrease;
		...
		if (!m_allowedToHitTarget)
		{
			...
			const CTimeValue fAmbientFireInc = gAIEnv.CVars.legacyPuppetRod.RODAmbientFireInc;
		...
		else if (m_targetZone == AIZONE_KILL)
		{
			...
			{
				const CTimeValue fKillZoneInc = gAIEnv.CVars.legacyPuppetRod.RODKillZoneInc;
...
CTimeValue CPuppet::GetFiringReactionTime(const Vec3& targetPos) const
{
	...
	CTimeValue fReactionTime = gAIEnv.CVars.legacyPuppetRod.RODReactionTime * GetParameters().m_PerceptionParams.reactionTime;
	...
	else
	{
		...
		const CTimeValue fDistInc = min(CTimeValue(1), gAIEnv.CVars.legacyPuppetRod.RODReactionDistInc);
		...
		if (fLeaningDot < thr1)
			...
			fReactionTime += gAIEnv.CVars.legacyPuppetRod.RODReactionDirInc * 2;
	}
