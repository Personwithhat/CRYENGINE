...
void CWaterWaveRenderNode::Update(float fDistanceToCamera)
{
	...
	float fDelta(m_pParams.m_fCurrSpeed * gEnv->pTimer->GetFrameTime().BADGetSeconds());
	...
	// Update lifetime
	m_pParams.m_fCurrFrameLifetime -= gEnv->pTimer->GetFrameTime();
	...
	m_fRECustomData[9] = clamp_tpl<float>(BADF(m_pParams.m_fCurrFrameLifetime / m_pParams.m_fCurrLifetime), 0.0f, 1.0f);
