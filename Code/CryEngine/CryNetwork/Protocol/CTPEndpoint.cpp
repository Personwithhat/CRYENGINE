...
void CCTPEndpoint::DumpOutputState(const CTimeValue& time)
{
	...
	fprintf(m_log_output, "Time:%s\n", time.GetMilliSeconds().str(6));
...
void CCTPEndpoint::ChannelStatsDraw()
{
	...
	y += (drawScale * 10.0f);
	m_queue.DrawLabel(0.0f, y, white, "%.2f", (float)m_PacketRateCalculator.GetPing(false).GetMilliSeconds());
	m_queue.DrawLabel(drawScale * 40.0f, y, white, "[%.2f]", (float)m_PacketRateCalculator.GetPing(true).GetMilliSeconds());
