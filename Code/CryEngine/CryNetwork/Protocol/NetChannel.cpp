...
void CNetChannel::Update(bool finalUpdate, CTimeValue nTime)
{
	...
	if (m_nKeepAliveTimer != CTimeValue(0) && nTime > m_nKeepAlivePingTimer + CNetCVars::Get().KeepAliveTime && !IsLocal())
	{
...
void CNetChannel::OnEndpointEvent(const SCTPEndpointEvent& evt)
{
	...
	case eCEE_BackoffTooLong:
		if (m_nKeepAliveTimer != CTimeValue(0) && (g_time - m_nKeepAliveTimer2).GetSeconds() < 20)
			GotPacket(eGPT_Fake);
		break;
	}
}
