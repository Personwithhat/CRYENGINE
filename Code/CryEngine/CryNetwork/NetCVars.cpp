...
CNetCVars::CNetCVars()
{
	...
	REGISTER_CVAR2_DEDI_ONLY("net_scheduler_expiration_period", &SchedulerSendExpirationPeriod, CTimeValue(1), VF_NULL, "Scheduler send queue expiration period in seconds (used for bandwidth tracking)");
...
#else
	REGISTER_CVAR2("net_inactivitytimeout", &InactivityTimeout, CTimeValue(30), 0, "Sets how many seconds without receiving a packet a connection will stay alive for (can only be set on server)");
	REGISTER_CVAR2("net_inactivitytimeoutDevmode", &InactivityTimeoutDevmode, CTimeValue(30), 0, "Sets how many seconds without receiving a packet a connection will stay alive for while in devmode (can only be set on server)");
#endif
	REGISTER_CVAR2_DEV_ONLY("net_backofftimeout", &BackoffTimeout, CTimeValue(6 * 60), 0, "Maximum time to allow a remote machine to stall for before disconnecting");
...
#endif // NEW_BANDWIDTH_MANAGEMENT
	REGISTER_CVAR2_DEV_ONLY("net_ping_time", &PingTime, CTimeValue(5), 0, "Controls the length of time between ping packets.");
	REGISTER_CVAR2_DEV_ONLY("net_keepalive_time", &KeepAliveTime, CTimeValue(10), 0, "Controls the length of time between keepalive packets.");
	...
	REGISTER_CVAR2_DEV_ONLY("net_channelLocalSleepTime", &channelLocalSleepTime, CTimeValue("0.01"), VF_DUMPTODISK, "Sleep time on a local channel. Maximum number of local packets per second = 1/sleepTime.");
	#else
...
//#endif // defined(DEDICATED_SERVER)
	...
	REGISTER_CVAR_CB_DEV_ONLY(net_PacketLagMin, CTimeValue(0), VF_NULL, "", OnPacketExtraLagChange);
	REGISTER_CVAR_CB_DEV_ONLY(net_PacketLagMax, CTimeValue(0), VF_NULL, "", OnPacketExtraLagChange);
	...
	REGISTER_CVAR2_DEV_ONLY("net_highlatencythreshold", &HighLatencyThreshold, CTimeValue("0.5"), VF_NULL, "");   // must be net synched
	REGISTER_CVAR2_DEV_ONLY("net_highlatencytimelimit", &HighLatencyTimeLimit, CTimeValue("1.5"), VF_NULL, "");   // must be net synched
	...
	REGISTER_CVAR2_DEV_ONLY("net_connectivity_detection_interval", &NetworkConnectivityDetectionInterval, CTimeValue(1), VF_NULL, "");
	...
	REGISTER_CVAR2_DEV_ONLY("net_fragment_expiration_time", &net_fragment_expiration_time, CTimeValue(1), VF_NULL, "Expiration time for stale fragmented packets");
