...
void CD3D9Renderer::DebugPerfBars(const SRenderStatistics& RStats, int nX, int nY)
{
	...
	fWaitForGPU = ((rtTimings.waitForGPU_MT + rtTimings.waitForGPU_RT).BADGetSeconds() + fWaitForGPU * fSmooth) / (fSmooth + 1.0f);
...
void CD3D9Renderer::RT_EndFrame()
{
	...
	SRenderStatistics::Write().m_Summary.waitForGPU_RT += (GTimer(d3d)->GetAsyncTime() - timePresentBegin);
	...
	gEnv->GetJobManager()->SetRenderDoneTime(GetGTimer()->GetAsyncTime());
#if defined(ENABLE_SIMPLE_GPU_TIMERS)
