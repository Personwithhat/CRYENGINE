...
void CGraphicsDG::Write(IStatoscopeFrameRecord& fr)
{
	...
	{
		TrackGPUUsage(GPUUsageInPercent, GetGTimer()->GetRealFrameTime(), numDrawCalls + numShadowDrawCalls);
	}
}
...
void CPerformanceOverviewDG::Write(IStatoscopeFrameRecord& fr)
{
	...
	fr.AddValue(BADF frameLength.GetMilliSeconds() - profilingCostMs);
	fr.AddValue((frameLength - renderTimes.fWaitForRender).GetMilliSeconds());
	fr.AddValue((renderTimes.fTimeProcessedRT - renderTimes.fWaitForGPU_MT - renderTimes.fWaitForGPU_RT).GetMilliSeconds());
