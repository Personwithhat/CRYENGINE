...
bool CWaterRipplesStage::RefreshParameters()
{
	...
	const CTimeValue fTime = GetGraphicsPipeline().GetAnimationTime();
...
bool CWaterRipplesStage::IsVisible() const
{
	...
	const bool bSimTimeOut = (gEnv->pTimer->GetFrameStartTime() - m_lastSpawnTime) > fTimeOut;
...
void CWaterRipplesStage::UpdateAndDrawDebugInfo()
{
	...
	const CTimeValue frameTime = std::max(CTimeValue(0), gEnv->pTimer->GetFrameTime());
