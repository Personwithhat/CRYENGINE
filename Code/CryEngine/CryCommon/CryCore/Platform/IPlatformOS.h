...
protected:
	...
	public:
		...
		void Reset()
		{
			...
			m_resetTime = (float)gEnv->pTimer->GetAsyncCurTime().GetSeconds();
		}
		...
		bool HasAchievedStableFPS()
		{
			...
			float fps = (float)gEnv->pTimer->GetFrameTime(true).GetSeconds();
			if (fps > 0.000001f)
				fps = __fres(fps);

			// Float inaccuracy is fine, only used as a bool to approximate stable FPS.
			float curTime = (float)gEnv->pTimer->GetAsyncCurTime().GetSeconds();
			if (fps < m_minFPS)
			{
				Reset();
				return false;
