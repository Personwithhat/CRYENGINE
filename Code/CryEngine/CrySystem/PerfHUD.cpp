...
bool CPerfHUD::OnInputEvent(const SInputEvent& rInputEvent)
{
	    ...
	    rInputEvent.deviceIndex == 0)
	{
		...
		else if (rInputEvent.state == eIS_Down)
		{
			...
			if (m_triggersDownStartTime > 0 && gEnv->pTimer->GetAsyncCurTime() - m_triggersDownStartTime > activateTime)
			{
...
void CFpsWidget::Update()
{
	...
	//
	{
		CTimeValue frameTime = gEnv->pTimer->GetRealFrameTime();
...
void CRenderStatsWidget::Update()
{
	...
	// FPS
	//
	m_runtimeData.fps = min(rTime(9999), gEnv->pTimer->GetFrameRate());
...
void CWarningsWidget::Update()
{
	...
	CTimeValue frameTime = gEnv->pTimer->GetRealFrameTime().GetSeconds();
