...
//////////////////////////////////////////////////////////////////////////
void CSystem::RenderBegin(uintptr_t hWnd)
{
	...
	if (rndAvail)
	{
		m_env.pRenderer->BeginFrame(hWnd);
	...
	if (pWorld && GetIRenderer())
	{
		...
		if (pVars->bProfileEntities == 1)
		{
			...
			for (i = 0; i < nEnts; i++)
			{
				...
				cry_sprintf(msgbuf, "%.2fms/%.1f (peak %.2fms/%d) %s (id %d)",
				  dt = (float)gEnv->pTimer->TicksToTime(pInfos[i].nTicksAvg).GetMilliSeconds(), pInfos[i].nCallsAvg,
				  (float)gEnv->pTimer->TicksToTime(pInfos[i].nTicksPeak).GetMilliSeconds(), pInfos[i].nCallsPeak,
		...
		if (pVars->bProfileFunx)
		{
			...
			for (j = 0, ++i; j < nFunx; j++, i++)
			{
				...
				IRenderAuxText::Draw2dLabel(renderMarginX, renderMarginY + i * lineSize, fontSize, fColor, false,
				  "%s %.2fms/%d (peak %.2fms/%d)", pInfos[j].pName, (float)gEnv->pTimer->TicksToTime(pInfos[j].nTicks).GetMilliSeconds(), pInfos[j].nCalls,
				  (float)gEnv->pTimer->TicksToTime(pInfos[j].nTicksPeak).GetMilliSeconds(), pInfos[j].nCallsPeak);
		...
		if (pVars->bProfileGroups)
		{
			...
			for (++i; j < nGroups; j++, i++)
			{
				...
				CTimeValue time = gEnv->pTimer->TicksToTime(pInfos[j].nTicksAvg);
...
void CSystem::RenderStats()
{
	...
	CTimeValue fFrameTime = gEnv->pTimer->GetRealFrameTime();
