...
void CFrameProfileSystem::EndFrame()
{
	...
	mpfloat smoothFactor = 1 - gEnv->pTimer->GetProfileFrameBlending(&smoothTime);
	...
	for (auto const pFrameProfiler : * m_pProfilers)
	{
		...
		if (pFrameProfiler != nullptr && (!m_bSubsystemFilterEnabled || (m_bSubsystemFilterEnabled && pFrameProfiler->m_subsystem == (uint8)m_subsystemFilter)))
		{
			...
			// Float inaccuracy is fine, debug/profiling
			pFrameProfiler->m_displayedValue = pFrameProfiler->m_displayedValue * (float)smoothFactor + currentValue * (1.0f - (float)smoothFactor);
			float variance = square(currentValue - aveValue);
			pFrameProfiler->m_variance = pFrameProfiler->m_variance * (float)smoothFactor + variance * (1.0f - (float)smoothFactor);
	...
	m_frameSecAvg = m_frameSecAvg * smoothFactor + frameSec * (1 - smoothFactor);

	CTimeValue frameLostSec = gEnv->pTimer->TicksToTime(m_frameTime - selfAccountedTime);
	m_frameLostSecAvg = m_frameLostSecAvg * smoothFactor + frameLostSec * (1 - smoothFactor);

	CTimeValue overheadtime = gEnv->pTimer->TicksToTime(numProfileCalls * m_nCallOverheadTotal / m_nCallOverheadCalls);
	m_frameOverheadSecAvg = m_frameOverheadSecAvg * smoothFactor + overheadtime * (1 - smoothFactor);
