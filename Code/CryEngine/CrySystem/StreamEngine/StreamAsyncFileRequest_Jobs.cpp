...
void CAsyncIOFileRequest::DecompressBlockEntry(SStreamJobEngineState engineState, int nJob)
{
	...
	if (!bFailed)
	{
		...
		if (readStatus == Z_OK || readStatus == Z_STREAM_END)
		{
			...
			m_unzipTime += gEnv->pTimer->TicksToTime(liEnd.QuadPart - liStart.QuadPart);
#endif
		}
		else
		{
...
void CAsyncIOFileRequest::DecryptBlockEntry(SStreamJobEngineState engineState, int nJob)
{
	...
	if (!bFailed)
	{
		...
		if (decryptOK)
		{
			...
			LARGE_INTEGER liEnd;
			QueryPerformanceCounter(&liEnd);
			m_decryptTime += gEnv->pTimer->TicksToTime(liEnd.QuadPart - liStart.QuadPart);
	#endif
		}
		else
		{
...
#if defined(STREAMENGINE_ENABLE_TIMING)
				QueryPerformanceCounter(&liEnd);
				m_verifyTime += gEnv->pTimer->TicksToTime(liEnd.QuadPart - liStart.QuadPart);
	#if defined(STREAMENGINE_ENABLE_STATS)
