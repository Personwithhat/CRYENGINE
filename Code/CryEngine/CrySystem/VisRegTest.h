...
public:
	...
	static const CTimeValue MaxStreamingWait;
...
protected:
	...
	struct SSample
	{
		...
		uint32      drawCalls;
		CTimeValue  gpuTimes[MaxNumGPUTimes];

		SSample() : imageName(NULL), frameTime(0), drawCalls(0)
		{
			for (uint32 i = 0; i < MaxNumGPUTimes; ++i)
				gpuTimes[i].SetSeconds(0);
		}
	};
