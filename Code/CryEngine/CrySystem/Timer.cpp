...
void CTimer::UpdateOnFrameStart()
{
	...
	/* PERSONAL NOTE: m_lBaseTime adjustment is a bad idea.
		'Rounding' errors and the like, hiding time-smoothing & scaling etc.
...
// PERSONAL VERIFY: Rewrite UpdatePostRollback using finalized UpdateOnFrameStart() as base
void CTimer::UpdatePostRollback(){
	/*if (!m_bEnabled)
		return;
	...
	// NOTE: No sleep or fixed_time_Step enforcing here
	...
	// NOTE:  Same as above more or less
		...
		//else
		//{
			// NOTE: CUMULATING FRAME TIME
			//float m_fFrameTime_OLD  = m_fFrameTime;

			// Clamp to max_time_step NOTE: cur-frame + prev-frame timestep has to be < 0.25 (max timestep) If < 0 set to 0 below.
			//m_fFrameTime = min(m_fRealFrameTime, m_max_time_step - m_fFrameTime_OLD);
		//}
	...
	// NOTE: Basicaly GetAsyncTime(), only we use the 'now' that the timer setups up as the 'real time at start of frame'
	m_realStartTime = CTimeValue(now * CTimeValue::TIMEVALUE_PRECISION / m_lTicksPerSec);
	
	
		The end result is a CTimeValue why convert to anything else in between?? etc.

		Then work on making the rest of the engine follow this....TicksToSeconds() is int64 to float conversion so probs wrong.

		int64 sampleTime = currentTime;

		// Get ASYNC time CTimeValue() 'value'
		int64 val1 = int64((sampleTime)* CTimeValue::TIMEVALUE_PRECISION * m_fSecsPerTick);

		// Get frame start time CTimeValue() 'value'
		int64 val2 = (int64)(float((double)(sampleTime) * m_fSecsPerTick) * CTimeValue::TIMEVALUE_PRECISION);

		CryLog("V1 %d V2 %d  Dif %3d", val1, val2, val1 - val2);
	...
	if (m_TimeDebug > 1)
	{
		//CryLogAlways("[CTimer]: Frame=%d Cur=%lld Now=%lld Off=%lld Async=%f CurrTime=%f UI=%f", gEnv->pRenderer->GetFrameID(false), (long long)currentTime, (long long)now, (long long)m_lOffsetTime, GetAsyncCurTime(), GetFrameStartTime(ETIMER_GAME), GetFrameStartTime(ETIMER_UI));
	}

	// NOTE: CUMULATING FRAME TIME, since rollback != actual simulation.
	//m_fFrameTime += m_fFrameTime_OLD;
