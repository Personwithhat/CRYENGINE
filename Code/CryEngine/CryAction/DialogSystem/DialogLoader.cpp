			...
			case ATTR_FACIAL:
				if (bLineValid)
				{
					...
					else
					{
						...
						if (nGood != 1 && nGood != 2)
						{
							GameWarning("[DIALOG] CDialogLoader::LoadFromTable: DialogScript '%s' has invalid Facial Expression Content '%s'. Using weight=%f fadetime=%s.", groupName.c_str(), szContent, w, tmp);
						}
						scriptLine.facialWeight = w;
						scriptLine.facialFadeTime.SetSeconds(tmp.c_str());
					}
				}
				break;
			case ATTR_LOOKAT:
