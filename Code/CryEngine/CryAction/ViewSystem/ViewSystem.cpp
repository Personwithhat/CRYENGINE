		...
		if (bIsActive)
		{
			...
			else
			{
				...
				if (m_fCameraNoise > 0)
				{
					...
					float fRes = pNoise->Noise1D(gEnv->pTimer->GetFrameStartTime().BADGetSeconds() * m_fCameraNoiseFrequency);
					aAng1.x += fRes * m_fCameraNoise * fScale;
					pos.z -= fRes * m_fCameraNoise * fScale;
					fRes = pNoise->Noise1D(17 + gEnv->pTimer->GetFrameStartTime().BADGetSeconds() * m_fCameraNoiseFrequency);
